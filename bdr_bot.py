...
def get_recent_podcast_feeds(hours=24, limit=100):
    ...
    try:
        ...
        headers = {
            ...
            "X-Podcastindex-Auth": os.getenv("PODCASTINDEX_API_KEY")
        }
        ...
        print(f"Fetching from Podcast Index... (last {hours} hours)")
...
def run_daily_bdr_scan():
    ...
    if not feeds:
        print("No feeds fetched. Check API key and Podcast Index status.")
        return
    ...
    for i, feed in enumerate(feeds[:20]):  # Test on first 20
        ...
        if classification.get("is_brand_show") and classification.get("confidence", 0) > 0.6:
            print(f"  ✓ Qualified ({classification.get('confidence', 0):.0%})")
    ...
    print(f"\n✓ Complete. Found {len(qualified_leads)} qualified leads")
